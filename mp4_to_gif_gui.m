...
%       输入/输出预览、文件大小预估
    ...
    inputFirstFrame = [];  % 缓存输入视频第一帧，用于输出预览
    ...
    hOutputEdit = uicontrol('Style', 'edit', 'String', '', ...
              ...
              'Callback', @cbOutputEdited);
    uicontrol('Style', 'pushbutton', 'String', '浏览...', ...
              'Position', [405, y0, 100, 26], ...
              'Callback', @cbBrowseOutput);
    ...
    hShortestSide = uicontrol('Style', 'edit', 'String', '300', ...
              ...
              'Callback', @cbShortestSideChanged);
    ...
    hWidthEdit = uicontrol('Style', 'edit', 'String', '', ...
              ...
              'HorizontalAlignment', 'center', 'BackgroundColor', 'white', ...
              'Visible', 'off', 'Callback', @cbManualSizeChanged);
    ...
    function cbBrowseInput(~, ~)
        ...
        try
            ...
            set(hVideoInfo, 'String', ...
                sprintf('视频: %d x %d | %.1f 秒 | %.1f fps | ~%d 帧', ...
    ...
    function cbPresetChanged(~, ~)
        ...
        switch val
            case 1, set(hRatioCheck, 'Value', 1); set(hShortestSide, 'String', '300'); toggleRatioUI(true);
            case 2, set(hRatioCheck, 'Value', 1); set(hShortestSide, 'String', '200'); toggleRatioUI(true);
            case 3, set(hRatioCheck, 'Value', 1); set(hShortestSide, 'String', '480'); toggleRatioUI(true);
            case 4, set(hRatioCheck, 'Value', 1); set(hShortestSide, 'String', '720'); toggleRatioUI(true);
            case 5, set(hRatioCheck, 'Value', 0); toggleRatioUI(false);
                if V.width > 0
                    set(hWidthEdit, 'String', num2str(V.width));
                    set(hHeightEdit, 'String', num2str(V.height));
                end
    ...
    function toggleRatioUI(maintainRatio)
        if maintainRatio
            set(hLabelSS, 'Visible', 'on'); set(hShortestSide, 'Visible', 'on'); set(hLabelSSpx, 'Visible', 'on');
            set(hLabelW, 'Visible', 'off'); set(hWidthEdit, 'Visible', 'off');
            set(hLabelH, 'Visible', 'off'); set(hHeightEdit, 'Visible', 'off'); set(hLabelWHpx, 'Visible', 'off');
        else
            set(hLabelSS, 'Visible', 'off'); set(hShortestSide, 'Visible', 'off'); set(hLabelSSpx, 'Visible', 'off');
            set(hLabelW, 'Visible', 'on'); set(hWidthEdit, 'Visible', 'on');
            set(hLabelH, 'Visible', 'on'); set(hHeightEdit, 'Visible', 'on'); set(hLabelWHpx, 'Visible', 'on');
        end
    end

    function cbShortestSideChanged(~, ~)
        set(hPreset, 'Value', 6); updateSizeInfo();
    end
    ...
    function updateSizeInfo()
        if V.width == 0
            set(hSizeInfo, 'String', '输出尺寸: -- x --    预计大小: --');
            return;
        end
        if get(hRatioCheck, 'Value')
            ...
            if V.width <= V.height
                outW = round(ss); outH = round(V.height * (ss / V.width));
            else
                outH = round(ss); outW = round(V.width * (ss / V.height));
            end
        else
            ...
            outW = round(outW); outH = round(outH);
        end
        ...
        switch qIdx
            case 1, compRatio = 5.0; case 2, compRatio = 6.5;
            case 3, compRatio = 8.5; case 4, compRatio = 11.0;
        end
        estBytes = outW * outH * totalOutFrames / compRatio;
        if estBytes < 1024
            estStr = sprintf('%.0f B', estBytes);
        elseif estBytes < 1024*1024
            estStr = sprintf('%.1f KB', estBytes/1024);
        else
            estStr = sprintf('%.2f MB', estBytes/(1024*1024));
        end
        set(hSizeInfo, 'String', ...
            sprintf('输出尺寸: %d x %d    预计大小: %s  (%d 帧)', ...
                    outW, outH, estStr, round(totalOutFrames)));
    end

    function showInputPreview(frame)
        axes(axInput); cla; imshow(frame, 'Parent', axInput, 'Border', 'tight');
    ...
    function showOutputPreview(frame)
        axes(axOutput); cla; imshow(frame, 'Parent', axOutput, 'Border', 'tight');
    ...
    function cbStartConversion(~, ~)
        if isempty(inputFile) || ~isfile(inputFile)
            errordlg('请选择有效的输入视频文件。', '输入错误'); return;
        end
        if isempty(outputFile)
            errordlg('请指定输出 GIF 文件。', '输出错误'); return;
        end
        targetFPS = str2double(get(hFpsEdit, 'String'));
        if isnan(targetFPS) || targetFPS <= 0 || targetFPS > 60
            errordlg('帧率必须在 1 到 60 fps 之间。', '参数错误'); return;
        end
        if get(hRatioCheck, 'Value')
            ss = str2double(get(hShortestSide, 'String'));
            if isnan(ss) || ss <= 0 || ss > 4096
                errordlg('最短边必须在 1 到 4096 像素之间。', '参数错误'); return;
            end
            if V.width <= V.height
                outW = round(ss); outH = round(V.height * (ss / V.width));
            else
            ...
            outH = str2double(get(hHeightEdit, 'String'));
            if isnan(outW) || isnan(outH) || outW <= 0 || outH <= 0
                errordlg('宽度和高度必须为正整数。', '参数错误'); return;
            end
            outW = round(outW); outH = round(outH);
        end
        qIdx = get(hQualityMenu, 'Value');
        ...
        set(hProgBg, 'Visible', 'on'); set(hProgBar, 'Visible', 'on');
        setProg(0);
        ...
        try
            ...
            set(hStatus, 'String', ...
                sprintf('转换中... %d x %d, %d色, %.1f fps', outW, outH, numColors, targetFPS));
            drawnow;
            v.CurrentTime = 0; frameIdx = 0; written = 0;
            while hasFrame(v)
                frame = readFrame(v);
                if mod(frameIdx, skipInterval) ~= 0
                    frameIdx = frameIdx + 1; continue;
                end
                ...
                if written == 0
                    imwrite(indImg, cmap, outputFile, 'DelayTime', 1/targetFPS, 'LoopCount', inf);
                else
                    imwrite(indImg, cmap, outputFile, 'DelayTime', 1/targetFPS, 'WriteMode', 'append');
                end
                written = written + 1; frameIdx = frameIdx + 1;
                if mod(written, 5) == 0
                    setProg(min(written / estOutputFrames, 1));
                    set(hStatus, 'String', ...
                        sprintf('进度: %d / ~%d 帧 (%.0f%%)', written, estOutputFrames, 100*written/estOutputFrames));
                    drawnow;
                end
            end
            setProg(1);
            ...
            if fInfo.bytes < 1024
                actualSize = sprintf('%.0f B', fInfo.bytes);
            elseif fInfo.bytes < 1024*1024
                actualSize = sprintf('%.1f KB', fInfo.bytes/1024);
            else
                actualSize = sprintf('%.2f MB', fInfo.bytes/(1024*1024));
            end
            set(hStatus, 'String', ...
                sprintf('完成! %d 帧, %d x %d, %.1f fps, 文件大小: %s  ->  %s', ...
            ...
            set(hProgBg, 'Visible', 'off'); set(hProgBar, 'Visible', 'off');
